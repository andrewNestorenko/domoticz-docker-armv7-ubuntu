...
if [ -x "$(command -v apt-get)" ]; then
	...
	domoticz_DEPS=( curl libcurl3 unzip wget sudo cron libudev-dev)
	...
	domoticz_DEPS=( curl libcurl3 unzip wget findutils cronie sudo domoticz_DEP)
...
# Check arguments for the undocumented flags
	for var in "$@"; do
		case "$var" in
			...
			"--i_do_not_follow_recommendations"   ) skn
